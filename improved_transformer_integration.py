...
from improved_transformer_model import ImprovedStockTransformer, ImprovedTransformerDirectionalLoss
import warnings
...
def train_improved_transformer_model(model, X_train, y_train, X_val, y_val, epochs=200, patience=40):
    """Обучение улучшенной Transformer модели с ранней остановкой"""
    ...
    # Используем улучшенную функцию потерь для DA
    criterion = ImprovedTransformerDirectionalLoss(mse_weight=0.1, da_weight=0.7, trend_weight=0.15, confidence_weight=0.05)
    ...
    print("\nНачинаем обучение улучшенного Transformer с ранней остановкой по DA...")
    for epoch in range(epochs):
        ...
        da_improved = current_da > best_da + 0.05  # уменьшенный порог для более частых обновлений
...
def plot_results_with_changes(model, X_test, y_test, dates_test, base_prices_test, original_data):
    ...
    plt.title('AAPL: Реальная цена High vs Предсказание Улучшенного Transformer', fontsize=14)
    plt.ylabel('Цена ($)')
    plt.legend()
    ...
    plt.title('Ошибки предсказания Улучшенного Transformer')
    plt.ylabel('Ошибка ($)')
    plt.xlabel('Дата')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('improved_transformer_stock_predictions.png', dpi=300, bbox_inches='tight')
    plt.show()  # Показываем график в Colab
    print("График сохранен как 'improved_transformer_stock_predictions.png'")
...
def run_improved_transformer_experiment():
    """Запуск эксперимента с улучшенной Transformer моделью"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # Создаем улучшенную Transformer модель
    ...
    print(f"\nУлучшенная Transformer модель создана: {sum(p.numel() for p in model.parameters()):,} параметров")

    # Обучаем модель
    train_losses, val_losses, val_das = train_improved_transformer_model(
        model, X_train, y_train, X_val, y_val, epochs=200, patience=40
    )
    ...
    print(f"\n✅ Обучение и тестирование улучшенного Transformer завершено!")
...
if __name__ == "__main__":
    model, da = run_improved_transformer_experiment()
