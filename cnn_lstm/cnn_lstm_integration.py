...
def train_cnn_lstm_model(model, X_train, y_train, X_val, y_val, epochs=150, patience=30):
...
def plot_results_with_changes(model, X_test, y_test, dates_test, base_prices_test, original_data):
    ...
    plt.title('AAPL: Реальная цена High vs Предсказание CNN+LSTM', fontsize=14)
    plt.ylabel('Цена ($)')
    plt.legend()
    ...
    plt.title('Ошибки предсказания CNN+LSTM')
    plt.ylabel('Ошибка ($)')
    plt.xlabel('Дата')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('cnn_lstm_stock_predictions.png', dpi=30, bbox_inches='tight')
    plt.show()  # Показываем график в Colab
    print("График сохранен как 'cnn_lstm_stock_predictions.png'")
...
def run_cnn_lstm_experiment():
    """Запуск эксперимента с CNN+LSTM моделью"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    print(f"\nCNN+LSTM модель создана: {sum(p.numel() for p in model.parameters()):,} параметров")

    # Обучаем модель
    train_losses, val_losses, val_das = train_cnn_lstm_model(
...
if __name__ == "__main__":
    model, da = run_cnn_lstm_experiment()
