...
def train_dynamic_weighted_cnn_lstm_model(model, X_train, y_train, X_val, y_val, epochs=150, patience=30):
    """Обучение CNN+LSTM модели с динамическим взвешиванием потерь"""
    ...
    # Используем динамически взвешенную функцию потерь
    criterion = DynamicWeightedDirectionalLoss(initial_pos_weight=1.0, initial_neg_weight=1.0,
    ...
    print("\nНачинаем обучение CNN+LSTM с динамическим взвешиванием потерь...")
...
def plot_results_with_changes(model, X_test, y_test, dates_test, base_prices_test, original_data):
    ...
    plt.title('AAPL: Реальная цена High vs Предсказание CNN+LSTM с динамическим взвешиванием', fontsize=14)
    plt.ylabel('Цена ($)')
    plt.legend()
    ...
    plt.title('Ошибки предсказания CNN+LSTM с динамическим взвешиванием')
    plt.ylabel('Ошибка ($)')
    plt.xlabel('Дата')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('cnn_lstm/dynamic_weighted_cnn_lstm_stock_predictions.png', dpi=300, bbox_inches='tight')
    plt.show()  # Показываем график в Colab
    print("График сохранен как 'cnn_lstm/dynamic_weighted_cnn_lstm_stock_predictions.png'")
...
def run_dynamic_weighted_cnn_lstm_experiment():
    """Запуск эксперимента с CNN+LSTM моделью с динамическим взвешиванием"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # Обучаем модель с динамическим взвешиванием
    train_losses, val_losses, val_das = train_dynamic_weighted_cnn_lstm_model(
    ...
    print(f"\n✅ Обучение и тестирование CNN+LSTM с динамическим взвешиванием завершено!")
...
if __name__ == "__main__":
    model, da = run_dynamic_weighted_cnn_lstm_experiment()
