# Детальная реализация анализа важности признаков

## Шаг 1: Модификация существующей модели для извлечения attention weights

### 1.1 Изменение класса ImprovedStockPredictor

Необходимо модифицировать `stock_predictor_hi.py` для извлечения attention weights:
...
## Шаг 2: Создание базового анализатора на основе attention

### 2.1 Класс AttentionFeatureAnalyzer
...
## Шаг 3: Расширенные методы анализа

### 3.1 Gradient-based анализ
    ...
    def _evaluate_with_permuted_feature(self, data_loader, metric_fn, feature_idx):
        ...
        with torch.no_grad():
            for batch_x, batch_y in data_loader:
                ...
                feature_values = permuted_x[:, :, feature_idx].numpy()
                np.random.shuffle(feature_values.flatten())
                permuted_x[:, :, feature_idx] = torch.tensor(feature_values)

                permuted_x = permuted_x.to(self.device)
                predictions = self.model(permuted_x).cpu().numpy()
                all_predictions.extend(predictions.flatten())
                all_targets.extend(batch_y.numpy())
...
## Шаг 4: Комплексный анализатор

### 4.1 Комбинирование всех методов
...
## Шаг 5: Функции для Directional Accuracy
...
Этот план реализации позволяет начать с простого подхода на основе attention weights и постепенно расширять до комплексного анализа, комбинирующего несколько методов для оценки важности признаков.
