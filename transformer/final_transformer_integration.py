...
from final_transformer_model import FinalStockTransformer, FinalTransformerDirectionalLoss
import warnings
...
def train_final_transformer_model(model, X_train, y_train, X_val, y_val, epochs=200, patience=40):
    """Обучение финальной Transformer модели с ранней остановкой"""
    ...
    # Используем финальную функцию потерь для DA
    criterion = FinalTransformerDirectionalLoss(mse_weight=0.1, da_weight=0.65, trend_weight=0.2, confidence_weight=0.05)
...
def plot_results_with_changes(model, X_test, y_test, dates_test, base_prices_test, original_data):
    ...
    plt.title('AAPL: Реальная цена High vs Предсказание Финального Transformer', fontsize=14)
    plt.ylabel('Цена ($)')
    plt.legend()
    ...
    plt.title('Ошибки предсказания Финального Transformer')
    plt.ylabel('Ошибка ($)')
    plt.xlabel('Дата')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('final_transformer_stock_predictions.png', dpi=30, bbox_inches='tight')
    plt.show()  # Показываем график в Colab
    print("График сохранен как 'final_transformer_stock_predictions.png'")
...
def run_final_transformer_experiment():
    """Запуск эксперимента с финальной Transformer моделью"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # Создаем финальную Transformer модель
    model = FinalStockTransformer(
    ...
    print(f"\nФинальная Transformer модель создана: {sum(p.numel() for p in model.parameters()):,} параметров")

    # Обучаем модель
    train_losses, val_losses, val_das = train_final_transformer_model(
...
if __name__ == "__main__":
    model, da = run_final_transformer_experiment()
