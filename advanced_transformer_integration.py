...
from advanced_transformer_model import AdvancedStockTransformer, AdvancedTransformerDirectionalLoss
import warnings
...
def train_advanced_transformer_model(model, X_train, y_train, X_val, y_val, epochs=200, patience=40):
    """Обучение продвинутой Transformer модели с ранней остановкой"""
    ...
    # Используем продвинутую функцию потерь для DA
    criterion = AdvancedTransformerDirectionalLoss(mse_weight=0.05, da_weight=0.7, trend_weight=0.2, confidence_weight=0.05)
    ...
    print("\nНачинаем обучение продвинутого Transformer с ранней остановкой по DA...")
...
def plot_results_with_changes(model, X_test, y_test, dates_test, base_prices_test, original_data):
    ...
    plt.title('AAPL: Реальная цена High vs Предсказание Продвинутого Transformer', fontsize=14)
    plt.ylabel('Цена ($)')
    plt.legend()
    ...
    plt.title('Ошибки предсказания Продвинутого Transformer')
    plt.ylabel('Ошибка ($)')
    plt.xlabel('Дата')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('advanced_transformer_stock_predictions.png', dpi=30, bbox_inches='tight')
    plt.show()  # Показываем график в Colab
    print("График сохранен как 'advanced_transformer_stock_predictions.png'")
...
def run_advanced_transformer_experiment():
    """Запуск эксперимента с продвинутой Transformer моделью"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # Создаем продвинутую Transformer модель
    ...
    print(f"\nПродвинутая Transformer модель создана: {sum(p.numel() for p in model.parameters()):,} параметров")

    # Обучаем модель
    train_losses, val_losses, val_das = train_advanced_transformer_model(
    ...
    print(f"\n✅ Обучение и тестирование продвинутого Transformer завершено!")
...
if __name__ == "__main__":
    model, da = run_advanced_transformer_experiment()
