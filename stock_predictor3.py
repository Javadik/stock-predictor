...
class StockPredictor(nn.Module):
    def __init__(self, input_size=10, hidden_size=128, num_layers=2, dropout=0.4):
        super().__init__()
...
class BalancedLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # Штраф за несбалансированные предсказания
        pred_mean = torch.mean(pred)
        balance_penalty = torch.abs(pred_mean)  # Штрафуем смещение предсказаний от нуля

        return mse_loss + self.balance_weight * balance_penalty

def train_model_with_early_stopping(model, X_train, y_train, X_val, y_val, epochs=100, patience=25):
    ...
    #criterion = nn.MSELoss()  # Вернули MSE для стабильности
    criterion = BalancedLoss(balance_weight=0.1)
    ...
    if best_model_state is not None:
        ...
        #print(f"Лучший Val DA: {best_da:.3f} (эпоха {best_da_epoch})")
...
if __name__ == "__main__":
    ...
    # Создаем модель
    model = StockPredictor(input_size=10, hidden_size=128, num_layers=2, dropout=0.4).to(device)
    ...
    train_losses, val_losses, val_das = train_model_with_early_stopping(
        model, X_train, y_train, X_val, y_val, epochs=100, patience=25
    )
