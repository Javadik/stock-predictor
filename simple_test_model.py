...
class SimpleImprovedStockPredictor(nn.Module):
    def __init__(self, input_size=15, hidden_size=256, num_layers=2, dropout=0.3):
        super().__init__()
...
def prepare_data(symbol='AAPL', period='2y', seq_length=30):
    ...
    data['SMA_50'] = data['High'].rolling(window=50).mean()

    # Простое отношение без сложных операций
...
def main():
    ...
    X_train, X_test, y_train, y_test, scaler, test_data = prepare_data('AAPL', period='2y')

    print(f"Размеры данных:")
    print(f"  Обучающие: {X_train.shape}")
    print(f"  Тестовые: {X_test.shape}")

    # Создание модели с проверкой размерностей
    try:
        model = SimpleImprovedStockPredictor(input_size=14, hidden_size=256, num_layers=2, dropout=0.3).to(device)
